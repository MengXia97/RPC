...
void TcpConnection::sendMessage(std::string str){
    std::cout<<"send message:"<<str<<std::endl;
...
TcpClient::TcpClient(boost::asio::io_service & io, MainWindow* ui) :m_con(new TcpConnection(io)), ep(ip::address::from_string("127.0.0.1"), 6688){
	//sock_pt sock(new ip::tcp::socket(ios));
	m_con->addService(new EchoImplService(m_con, ui));
